...
loc.vector <- 1:25
    ...
    for(loc in 1:25){
        ...
        pdfname = paste0('plots/',region,'_validation/fits_l',loc,'.pdf')
        predname1 = paste0('fits/',region,'_validation/fits_temp_l',loc,'.RDS')
        predname2 = paste0('fits/',region,'_validation/fits_prcp_l',loc,'.RDS')
        
        y1_train <- c(obs.long$tmax[loc.vector==cur.loc & gcm.years <= 2000],
                      gcm.long$tmax[loc.vector==cur.loc & gcm.years <= 2000])
        y2_train <- c(  obs.long$pr[loc.vector==cur.loc & gcm.years <= 2000],
                        gcm.long$pr[loc.vector==cur.loc & gcm.years <= 2000])
        y1_test  <- c(obs.long$tmax[loc.vector==cur.loc & gcm.years > 2000],
                      gcm.long$tmax[loc.vector==cur.loc & gcm.years > 2000])
        y2_test  <- c(  obs.long$pr[loc.vector==cur.loc & gcm.years > 2000],
                        gcm.long$pr[loc.vector==cur.loc & gcm.years > 2000])
        y2_train <- log(0.001+y2_train)
        y2_test <- log(0.001+y2_test)
        ...
        if(loc>1){
            for(k in nns){
                x.vec = c(obs.long$tmax[loc.vector==k & gcm.years <= 2000],
                          gcm.long$tmax[loc.vector==k & gcm.years <= 2000])
        ...
        if(loc>1){
            for(k in nns){
                x.vec = c(obs.long$pr[loc.vector==k & gcm.years <= 2000],
                          gcm.long$pr[loc.vector==k & gcm.years <= 2000])
                x.vec = log(0.001+x.vec)
 ...
 ############### Predictions temp     
        ...
        if(loc>1){
            X1_pred = X1_test[,1]
            for(k in nns){
                vecname = paste0('fits/',region,'_validation/fits_temp_l',k,'.RDS')
 ...
 ############### Predictions prcp        
        ...
        if(loc>1){
            ...
            nx3 <- ncol(X2_pred)+1
            for(k in nns){
                vecname = paste0('fits/',region,'_validation/fits_prcp_l',k,'.RDS')
                x.vec = readRDS(vecname)
