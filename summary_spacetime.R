...
for(mnth in 1:12){
    ...
    for(loc in 1:25){
        ...
        envname = paste0('fits/fits_spacetime/fits_temp_m',mnth,'_l',loc,'.RDS')
        qf.y1.mle.ts <- readRDS(envname)
        envname = paste0('fits/fits_spacetime/fits_prcp_m',mnth,'_l',loc,'.RDS')
...
save(y1y2.cors.0,y1y2.cors.1,cal.data,
           file = 'summary_spacetime.RData')
load('summary_spacetime.RData')
eachmonth = rep(NA,12)
# for(i in 1:12){
