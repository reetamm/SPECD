...
#         x21 = y21[c(n1,1:(n1-1))]
#         
#         envname = paste0('fits/fits_space/fits_temp_m',mnth,'_l',loc,'.RDS')
#         qf.y1.mle.ts <- readRDS(envname)
#         envname = paste0('fits/fits_space/fits_prcp_m',mnth,'_l',loc,'.RDS')
...
png('autocorr_space.png',width = 1200,height = 400)
par(mfrow=c(1,3))
plot(y1.cors.0,y1.cors.1,pch=20,xlab = 'calibrated',ylab = 'observed',main = 'temp autocorrelations')
abline(0,1)
plot(y2.cors.0,y2.cors.1,pch=20,xlab = 'calibrated',ylab = 'observed',main = 'prcp autocorrelations')
abline(0,1)
...
png('spatcorr_time.png',width = 800,height = 400)
corr_mat1 <- corr_mat2 <- corr_mat3 <- corr_mat4 <- NA
par(mfrow=c(1,2))
for(i in 1:12){
    cal.array <- cal.data[[i]]
    corr_mat1 <- c(corr_mat1, gdata::upperTriangle(cor(cal.array[,2,])))
    corr_mat2 <- c(corr_mat2, gdata::upperTriangle(cor(cal.array[,3,])))
    
    corr_mat3 <- c(corr_mat3, gdata::upperTriangle(cor(cal.array[,5,])))
    corr_mat4 <- c(corr_mat4, gdata::upperTriangle(cor(cal.array[,6,])))
    
}
plot(corr_mat1,corr_mat2,pch=20,xlab = 'calibrated',ylab='observed',main = 'temp spatial correlations')
abline(0,1)
plot(corr_mat3,corr_mat4,pch=20,xlab = 'calibrated',ylab='observed',main = 'prcp spatial correlations')
abline(0,1)
