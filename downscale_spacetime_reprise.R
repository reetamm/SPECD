...
for(mnth in 1:12)
    for(loc in 1:25){
        pdfname = paste0('plots/',region,'/plot_m',mnth,'_l',loc,'.pdf')
        modelname2 = paste0('fits/',region,'/fits_temp_m',mnth,'_l',loc)
        predname2 = paste0('fits/',region,'/fits_temp_m',mnth,'_l',loc,'.RDS')
        modelname1 = paste0('fits/',region,'/fits_prcp_m',mnth,'_l',loc)
        predname1 = paste0('fits/',region,'/fits_prcp_m',mnth,'_l',loc,'.RDS')
        
        y2 <- c(obs.long$tmax[vecchia.order==loc & gcm.months==mnth],gcm.long$tmax[vecchia.order==loc & gcm.months==mnth])
        y1 <- c(obs.long$pr[vecchia.order==loc & gcm.months==mnth],gcm.long$pr[vecchia.order==loc & gcm.months==mnth])
        y1 <- log(0.0001+y1)
 ...
 ############### Predictions prcp        
        ...
        if(loc>1){
            ...
            k.start = max(1,loc-5)
            for(k in k.start:k.end){
                vecname = paste0('fits/',region,'/fits_temp_m',mnth,'_l',loc-k,'.RDS')
                x.vec = readRDS(vecname)
