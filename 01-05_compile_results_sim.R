...
for(mnth in 1:1){
    ...
    for(loc in 1:25){
        if(train){
            ...
            qf.y1.mle   <- readRDS(envname)[train_indices]
            envname     <- paste0( 'fits/',sim,'/',dataset,'/fits_prcp_l',loc,'.RDS')
            qf.y2.mle   <- readRDS(envname)[train_indices]
            qf.y2.mle   <- exp(qf.y2.mle) - 0.0001
        }
        ...
        if(!train){
            ...
            envname     <- paste0( 'fits/',sim,'/',dataset,'/fits_temp_l',loc,'.RDS')
            qf.y1.mle   <- readRDS(envname)[test_indices]
            envname     <- paste0( 'fits/',sim,'/',dataset,'/fits_prcp_l',loc,'.RDS')
            qf.y2.mle   <- readRDS(envname)[test_indices]
            qf.y2.mle   <- exp(qf.y2.mle) - 0.0001
        }
        n0  <- n1 <- length(y1)/2
        n   <- n0 + n1
        ...
        y1y2.cors.0 <- c(y1y2.cors.0,cor(qf.y1.mle[y0==1],qf.y2.mle[y0==1]))

        cal.array[,1,loc] <- y1[y0==0]          #model
        cal.array[,2,loc] <- qf.y1.mle[y0==0]   #calibrated
        cal.array[,3,loc] <- y1[y0==1]          #obs
        cal.array[,4,loc] <- y2[y0==0]          #model
        cal.array[,5,loc] <- qf.y2.mle[y0==0]   #calibrated
