 ...
 ############### Predictions temp     
        ...
        if(loc>1){
            ...
            for(i in 1:n_test){
                ...
                if(i>n0_test+1)
                    x_pred = c(1,qf.y1[i-1],X1_pred_scaled[i,-c(1:2)])
                qf.y1[i] <- predict(fit.y1,   X = x_pred, type = "QF",tau=qout11[i])
            }   
        }
...
print("Predictions prcp")
        if(loc==1){
            ...
            for(i in 1:n_test){
                ...
                if(i>n0_test+1)
                    x_pred = c(1,qf.y1[i-1],qf.y1[i],qf.y2[i-1])
                qf.y2[i] <- predict(fit.y2,   X = x_pred, type = "QF",tau=qout21[i])
            } 
        }
        
        if(loc>1){
            X2_pred = cbind(X1_pred_scaled,qf.y1,X2_test_scaled[,nx1+1])
            nx3 <- ncol(X2_pred)+1
            for(k in nns){
            ...
            for(i in 1:n_test){
                ...
                if(i<=n0_test+1)
                    x_pred = c(1,X1_test_scaled[i,-1],y1_test_scaled[i],X2_test_scaled[i,-c(1:nx1)])
                if(i>n0_test+1)
                    x_pred = c(1,qf.y1[i-1],X1_pred_scaled[i,-c(1:2)],qf.y1[i],qf.y2[i-1],X2_pred_scaled[i,(nx1+2):nx2])
                qf.y2[i] <- predict(fit.y2,   X = x_pred, type = "QF",tau=qout21[i])
            }   
        }
