...
for(mnth in 1:12){
    ...
    for(loc in 1:25){
        ...
        envname = paste0('fits/',region,'_validation/fits_temp_l',loc,'.RDS')
        qf.y1.mle.ts <- readRDS(envname)
        pred.months <- gcm.months[gcm.years>2000 & loc.vector==loc]
        qf.y1.mle.ts <- qf.y1.mle.ts[pred.months==mnth]
        envname = paste0('fits/',region,'_validation/fits_prcp_l',loc,'.RDS')
        qf.y2.mle.ts <- readRDS(envname)
        qf.y2.mle.ts <- exp(qf.y2.mle.ts) - 0.001
        qf.y2.mle.ts <- qf.y2.mle.ts[pred.months==mnth]
...
d0 <-density(log(0.001+cal.array2[,4])) # gcm
d1 <-density(log(0.001+cal.array2[,6])) # obs 
d2 <- density(log(0.001+cal.array2[,5])) # pred
plotmax.y = max(d0$y,d1$y,d2$y)
plotmin.y = min(d0$y,d1$y,d2$y)
plotmax.x = max(d0$x,d1$x,d2$x)
plotmin.x = min(d0$x,d1$x,d2$x)
